...
namespace SharpBoot.Sockets.server
{
    ...
    public class SharpServer : IDisposable
    {
        ...
        ConcurrentDictionary<Socket, SharpChannel> _clientMap;
        ...
        public virtual event Action<SharpChannel> NewChannel;
        ...
        public SharpServer(int listenPort, int maxClient)
        ...
        public SharpServer(IPEndPoint localEP, int maxClient)
        ...
        /// <param name="listenPort">监听的端口</param>
        /// <param name="maxClient">最大客户端数量</param>
        public SharpServer(IPAddress localIPAddress, int listenPort, int maxClient)
        {
            ...
            _clientMap = new ConcurrentDictionary<Socket, SharpChannel>();
        }
            ...
            if (e.SocketError == SocketError.Success)
            {
                ...
                if (s.Connected)
                {
                    try
                    {
                        ...
                        SharpChannel sharpChannel = new SharpChannel(this, asyniar);
                        _clientMap.TryAdd(s, sharpChannel);
                        NewChannel?.Invoke(sharpChannel);
        ...
        private void ProcessReceive(SocketAsyncEventArgs e)
        {
            if (e.BytesTransferred > 0 && e.SocketError == SocketError.Success)//if (e.BytesTransferred > 0 && e.SocketError == SocketError.Success)
            {
                ...
                _clientMap.TryGetValue(s, out var channel);
        ...
        private void CloseClientSocket(Socket s, SocketAsyncEventArgs e)
        {
            try
            {
                bool flag = _clientMap.TryRemove(s, out SharpChannel channel);
                if (!flag) return;
                channel?.Dispose();
            }
            catch (Exception)
